...
function draw() {
  ...
  if (!(mouseX < gridOrigin.x && mouseY < 300) || !isLightFree) {
    ...
    for (let z = 0; z < clustersZ; ++z) {
      ...
      for (let y = 0; y < clustersY; ++y) {
        ...
        let isCulled = false;
        let circlePosition = lightPos;
        let radius = range / cos(angle);
        isCulled = TestSphereVsABBB(circlePosition, radius, center, extents);
    ...
    if (debugViewCheckbox.checked() == true) {
      stroke(255, 0, 0, 255);
      circle(lightPos_s.x, lightPos_s.y, range / cos(angle) * 2);
    }
  ...
  fill(0);
  textSize(18);
  text('Method: Simple Sphere vs AABB', spotlightRangeSlider.x, 30);
  textSize(16);
